...
const CATEGORY_META: Record<InboxCategory | "all", { label: string; icon: typeof MessageSquare; color: string }> = {
  ...
  fluff: { label: "Fluff", icon: Smile, color: "var(--fg-3)" },
      ...
      <div className="grid h-[calc(100vh-230px)] gap-5 lg:grid-cols-3">
        ...
        <div className="flex flex-col gap-4">
            ...
            {(["leads", "complaints", "fluff", "all"] as const).map((cat) => {
              ...
              return (
                <button key={cat} onClick={() => setActiveCategory(cat)} className="flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-[13px] font-medium transition-all"
                  style={active ? { borderColor: "rgba(99,102,241,0.5)", background: "rgba(99,102,241,0.12)", color: "#fff" } : { borderColor: "var(--stroke)", color: "var(--fg-2)" }}>
          ...
          <div className="flex-1 space-y-2 overflow-y-auto pr-1">
            {filtered.length === 0 ? (
              <div className="py-12 text-center"><MessageSquare className="mx-auto mb-2 h-8 w-8 text-[var(--fg-4)]" /><p className="text-sm text-[var(--fg-4)]">No messages here.</p></div>
            ) : filtered.map((msg) => (
              <div key={msg.id} onClick={() => setSelectedId(msg.id)} className="glass-panel cursor-pointer rounded-xl p-3 transition-all"
                ...
                <div className="mb-1.5 flex items-center gap-2">
                  <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full text-[11px] font-bold text-[var(--fg)]" style={{ background: "linear-gradient(135deg,#6366f1,#a855f7)" }}>{initials(msg.author_name)}</div>
                  <span className="flex-1 truncate text-sm font-medium text-[var(--fg)]">{msg.author_name ?? "Unknown"}</span>
                  {!msg.is_read && <span className="h-2 w-2 flex-shrink-0 rounded-full" style={{ background: "var(--sai-red)" }} />}
                  <span className="flex-shrink-0 text-[11px] text-[var(--fg-4)]">{new Date(msg.received_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
                </div>
                <p className="line-clamp-2 text-[12.5px] text-[var(--fg-3)]">{msg.message}</p>
                <div className="mt-1.5 flex items-center gap-2"><Pill tone={catTone(msg.category)}>{msg.category}</Pill><span className="text-[11px] text-[var(--fg-4)]">{msg.platform}</span></div>
              </div>
            ))}
          </div>
        </div>
        ...
        {selected ? (
          <div className="glass-panel flex flex-col overflow-hidden rounded-2xl lg:col-span-2">
            <div className="flex items-center gap-3 border-b border-[var(--stroke)] p-4">
              <div className="flex h-10 w-10 items-center justify-center rounded-full text-sm font-bold text-[var(--fg)]" style={{ background: "linear-gradient(135deg,#6366f1,#a855f7)" }}>{initials(selected.author_name)}</div>
              <div className="flex-1"><p className="font-medium text-[var(--fg)]">{selected.author_name ?? "Unknown"}</p><p className="text-[12px] text-[var(--fg-4)]">{selected.platform} · {new Date(selected.received_at).toLocaleString()}</p></div>
              <Pill tone={catTone(selected.category)}>{selected.category}</Pill>
            </div>

            <div className="flex-1 overflow-y-auto p-6">
              <div className="max-w-lg">
                <div className="rounded-xl border border-[var(--stroke)] bg-[var(--panel-fill)] p-4"><p className="text-sm leading-relaxed text-[var(--fg)]">{selected.message}</p></div>
                {selected.category === "leads" && (
                  <div className="mt-4 rounded-xl border border-[var(--sai-gold)]/20 bg-[var(--sai-gold)]/[0.06] p-4">
                    ...
                    <p className="text-sm leading-relaxed text-[var(--fg-2)]">Hi {selected.author_name?.split(" ")[0]}! Thanks for reaching out 🙌 {selected.reply_content || "I'd love to tell you more — let me send you a quick overview."}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="border-t border-[var(--stroke)] p-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <button onClick={send} className="rounded-full px-4 py-1.5 text-[12.5px] font-semibold text-[var(--fg)] transition-transform hover:scale-[1.03]" style={{ background: "linear-gradient(135deg,#6366f1,#a855f7)" }}>Send reply</button>
              </div>
            </div>
          </div>
        ) : (
