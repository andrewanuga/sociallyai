...
export default function SignupPage() {
  ...
  return (
    <div className="w-full max-w-md">
      ...
      <div className="p-8 rounded-2xl border border-border bg-card/80 backdrop-blur-sm shadow-xl">
        {/* Google SSO */}
        <Button type="button" variant="outline" className="w-full mb-6 h-11 gap-3" onClick={handleGoogleSignup}>
              ...
              <div className="space-y-1.5 mt-2">
                {PASSWORD_REQUIREMENTS.map((req, i) => (
                  <div key={i} className="flex items-center gap-2 text-xs">
                    <div className={`w-3.5 h-3.5 rounded-full flex items-center justify-center transition-colors ${req.test(password) ? "bg-red-500" : "bg-muted border border-border"}`}>
                      {req.test(password) && <Check className="w-2.5 h-2.5 text-white" />}
                    </div>
                    <span className={req.test(password) ? "text-red-400" : "text-muted-foreground"}>
                      {req.label}
                    </span>
                  </div>
                ))}
        ...
        <p className="text-center text-xs text-muted-foreground mt-4">
          ...
          <Link href="#" className="text-red-400 hover:underline">Terms</Link>
          {" "}and{" "}
